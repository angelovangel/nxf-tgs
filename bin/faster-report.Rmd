...
params:
 author:
  label: "Author"
  value: "KAUST Bioscience Core Laboratory"
  input: text
 fastq_dir:
...
Generated at `r params$author` on `r format.Date(Sys.time())`. All data is calculated with the <a href="https://github.com/angelovangel/faster" target="_blank">faster</a> and <a href="https://github.com/angelovangel/fastkmers" target="_blank">fastkmers</a> programs.    
...
dfcols <- c("file", "reads", "bases", "min_len", "max_len", "N50", "Q20", "Q30")
# } else {
...
df <- stats_df %>% 
	dplyr::mutate(file = basename(file) %>% tools::file_path_sans_ext(compression = T)) %>%
...
num_reads <- df %>% dplyr::summarise(seqs = sum(reads, na.rm = T)) %>% as.numeric()
...
num_bases <- df %>% dplyr::summarise(bases = sum(bases, na.rm = T)) %>% as.numeric()
num_bases_text <- system2("siformat.sh", args = num_bases, stdout = T)
q30 <- df %>% dplyr::summarise(q30 = mean(Q30, na.rm = T)) %>% as.numeric()
q30_text <- sprintf("%.2f%%", q30)
q20 <- df %>% dplyr::summarise(q20 = mean(Q20, na.rm = T)) %>% as.numeric()
q20_text <- sprintf("%.2f%%", q20)
...
<tr><td class="meta-label">Run date:</td><td class="meta-value"><b>`r params$rundate`</b></td></tr>
<tr><td class="meta-label">Platform:</td><td class="meta-value"><b>`r params$sequencer`</b></td></tr>
<tr><td class="meta-label">Flow cell:</td><td class="meta-value"><b>`r params$flowcell`</b></td></tr>
<tr><td class="meta-label">Basecall:</td><td class="meta-value"><b>[`r params$basecall`](https://software-docs.nanoporetech.com/dorado/latest/models/models/)</b></td></tr>
<tr><td class="meta-label">User:</td><td class="meta-value"><b>`r params$user`</b></td></tr>
</table>
<table class="meta-table">
<tr><td class="meta-label">Files:</td><td class="meta-value"><b>`r num_files`</b></td></tr>
<tr><td class="meta-label">Reads:</td><td class="meta-value"><b>`r num_reads_text`</b></td></tr>
<tr><td class="meta-label">Bases:</td><td class="meta-value"><b>`r num_bases_text`</b></td></tr>
<tr><td class="meta-label">Mean Q20:</td><td class="meta-value"><b>`r q20_text`</b></td></tr>
<tr><td class="meta-label">Mean Q30:</td><td class="meta-value"><b>`r q30_text`</b></td></tr>
</table>
</div>
...
htmltools::tagList(
  htmltools::div(
    ...
    #htmltools::tags$select(id = 'custom-numbers', lapply(c('No formatting', 'Thousand separator', 'SI units'), htmltools::tags$option))
  ),
  ...
  reactable::reactable(
    ...
    #searchable = T,
    columns = list(
      file = colDef(name = "File", minWidth = 150, 
        #html = TRUE,
        #cell = function(index) {
        #  as.character(bslib::tooltip(bsicons::bs_icon("info-circle"), 'test'))
        #}
      ),
      reads = colDef(name = "Reads", format = colFormat(separators = T)),
      bases = colDef(name = "Bases", format = colFormat(separators = T)),
      min_len = colDef(name = "Min len", format = colFormat(separators = T), minWidth = 50),
      max_len = colDef(name = "Max len", format = colFormat(separators = T), minWidth = 50),
      N50 = colDef(name = "N50",format = colFormat(separators = T), minWidth = 50),
      Q20 = colDef(name = "Q20 (% bases)",format = colFormat(suffix = '%', digits = 2), minWidth = 60),
      Q30 = colDef(name = "Q30 (% bases)",format = colFormat(suffix = '%', digits = 2), minWidth = 60)
    )
  )
)
...
spark_kmers <- function(kmers_tbl) {
  fillcolor <- "#5D6D7E"
  n <- nrow(kmers_tbl)
  bar_colors <- rep(fillcolor, n)
  bar_colors[seq(1, n, by = 16)] <- "red"
  spk_chr(
    kmers_tbl$counts,
    type = "bar",
    colorMap = bar_colors,
...
if (params$sequencer != "Illumina") {
  gc_df <- data.frame(
	file = basename(fastqfiles) %>% tools::file_path_sans_ext(compression = T),
...
gc_df %>%
  ...
  reactable(
    ...
    columns = list(
     ...
     gc_content_dist = colDef(name = "GC content"),
     content_percycle = colDef(name = "Content per cycle"),
     qscore_dist = colDef(name = "Q-score", minWidth = 200),
     qscore_percycle = colDef(name = "Q-score per cycle"),
     dupl_rate = colDef(name = "Dupl rate", minWidth = 60),
     base_yield_by_len = colDef(name = "Base yield by length", minWidth = 200),
     kmer_counts = colDef(name = "k-mer counts", minWidth = 200)
    )
  )
...
<p style="color: #6c757d; font-size: 0.9em; text-align: right;">
Github repository: <a href="https://github.com/angelovangel/faster-report" target="_blank" style="color: #4e5256ff;">faster-report</a> | commit: `r params$git_commit`
</p>
