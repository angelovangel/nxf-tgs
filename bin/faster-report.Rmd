---
title: |
  <img style="float: right;" width="150;" src="www/bcl2.png">  FASTQ report    
...
Generated at `r params$author` on `r format.Date(Sys.time())`.
...
All fastq data is calculated with the [faster](https://github.com/angelovangel/faster) and [fastkmers](https://github.com/angelovangel/fastkmers) programs.
...
nduplex_command <- function(x) {
  as.numeric(system2('./nduplex.sh', args = x, stdout = T))
}
...
spark_phred <- function(phred_density_obj) {
  ...
  fillcolor <- "#5D6D7E"
	spk_chr(round(phred_density_obj$y, digits = 2), 
					...
					colorMap = c(rep(fillcolor, 19), "red", rep(fillcolor, 9), "red", rep(fillcolor, 19), "red", rep(fillcolor, 19)),
					width = 140, height = 40,
...
spark_content <- function(content_tbl) {
  sl_gc <- sparkline(round(content_tbl$g + content_tbl$c, 3), 
  ...
  sl_at <- sparkline(round(content_tbl$a + content_tbl$t, 3), 
                    xvalues = content_tbl$cycle, 
                    lineColor = "#d7191c", 
                    lineWidth = 3,
                    fillColor = FALSE,
  ...
  sl_n <- sparkline(round(content_tbl$n, 3), 
                    xvalues = content_tbl$cycle, 
                    lineColor = "#5D6D7E", 
                    lineWidth = 3,
                    fillColor = FALSE,
...
spark_content_phred <- function(content_tbl) {
  spk_chr(round(content_tbl$mean_phred, 3), 
          xvalues = content_tbl$cycle, 
          type = "line", 
          lineWidth = 3,
          lineColor = "#2b83ba", 
          fillColor = FALSE, width = 140, height = 40, 
          chartRangeMin = 10, chartRangeMax = 50, 
          chartRangeClip = TRUE, 
          normalRangeMin = 0, normalRangeMax = 20, normalRangeColor = "#D0D3D4",
...
gc_density <- parallel::mcmapply(faster_gc, fastqfiles, 
                                 mc.cores = numcores,
                                 mc.preschedule = T, 
                                 SIMPLIFY = FALSE, 
                                 MoreArgs = list(saveraw = params$rawdata))
q_score_density <- parallel::mcmapply(faster_qscore, fastqfiles, 
                                      mc.cores = numcores,
                                      mc.preschedule = T, 
                                      SIMPLIFY = FALSE, 
                                      MoreArgs = list(saveraw = params$rawdata))
kmers_tbl_list <- parallel::mcmapply(fastkmers, fastqfiles, 
                                     mc.cores = numcores,
                                     mc.preschedule = T, 
                                     SIMPLIFY = FALSE, 
                                     MoreArgs = list(saveraw = params$rawdata))
...
if (params$sequencer != "Illumina") {
  len_density <- parallel::mcmapply(faster_len, fastqfiles, 
                                    mc.cores = numcores,
                                    mc.preschedule = T, 
                                    SIMPLIFY = FALSE, 
                                    MoreArgs = list(saveraw = params$rawdata))
...
if(params$sequencer == "Illumina") {
  ...
  duplevel_tbl <- parallel::mcmapply(duplevel, fastqfiles, 
                                     mc.cores = numcores, 
                                     mc.preschedule = T, 
                                     SIMPLIFY = FALSE, 
                                     MoreArgs = list(saveraw = params$rawdata))
  content_tbl <- parallel::mcmapply(content_percycle, fastqfiles, 
                                    mc.cores = numcores,
                                    mc.preschedule = T,
                                    SIMPLIFY = FALSE, 
                                    MoreArgs = list(saveraw = params$rawdata))
...
table2_caption <- ifelse(params$sequencer != "Illumina", 
                         "Density distributions of GC-content, 'mean' q-score, length and k-mer counts (k = 3). The q-scores 20 and 30 are in red", 
