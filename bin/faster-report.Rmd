...
kableExtra::kbl(
  df,
  digits = 2, escape = F,
  caption = 'Table 1. FASTQ files statistics', 
  format.args = list(big.mark = ",")
  ) %>%
  #column_spec(2, background = spec_color(df$reads, option = 'turbo', begin = 0.4, end = 0.6)) %>%
  # kable_styling(
...
spark_len <- function(len_density_obj) {
  spk_chr(
    #paste(len_density_obj$mids, ":", len_density_obj$counts, sep = ""),
    len_density_obj$counts * len_density_obj$mids,
    type = "bar",
    barColor = "#5D6D7E",
    #lineWidth = 3,
    #fillColor = "#D0D3D4",
    ...
    height = 40,
    tooltipFormatter = spk_tool(
      "base yield for length ", 
      paste0(round(len_density_obj$breaks, 0), ' to ', round(len_density_obj$breaks + 832, 0)),
      prettyNum(
        round(len_density_obj$counts * len_density_obj$mids, 0), big.mark = ","
      )
    )
...
table2_caption <- ifelse(
  ...
  "Density distributions of GC-content, 'mean' q-score, length and k-mer counts (k = 3). The q-scores 10, 20, 30, 40 are in red. Lengths > 50 kb are binned to 50 kb.",
...
gc_df %>%
  ...
  kableExtra::kbl(escape = F,
                  caption = paste0("Table2. ", table2_caption)) %>%
